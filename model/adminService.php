...
{
   return admin_Get("SELECT admin.Id, admin.TenDangNhap, admin.MatKhau FROM  admin ".(($where!='')?(' where '.$where):'')." Order By ".$Order." Limit ".(($CurrentPage-1)*$PageSize)." , ".$PageSize);
}
//
function admin_insert($obj)
{
    return exe_query("insert into admin (TenDangNhap,MatKhau) values ('$obj->TenDangNhap','$obj->MatKhau')",'admin');
}
//
function admin_update($obj)
{
    return exe_query("update admin set TenDangNhap='$obj->TenDangNhap',MatKhau='$obj->MatKhau' where Id=$obj->Id",'admin');
}
//
